...
def test_ewise_fn(fn, shape, device):
    ...
    A = ltt.Tensor(nd.array(_A), device=device)
    B = ltt.Tensor(nd.array(_B), device=device)
...
def test_log(shape, device):
    ...
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.log(_A), ltt.log(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_exp(shape, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.exp(_A), ltt.exp(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_relu(shape, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.maximum(_A, 0), ltt.relu(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_tanh(shape, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.tanh(_A), ltt.tanh(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_stack(shape, axis, l, device):
    ...
    A = [ltt.Tensor(nd.array(_A[i]), device=device) for i in range(l)]
    A_t = [torch.Tensor(_A[i]) for i in range(l)]
    out = ltt.stack(A, axis=axis)
...
def test_stack_backward(shape, axis, l, device):
    ...
    ltt.stack(A, axis=axis).sum().backward()
...
def test_summation(shape, axes, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.sum(_A, axes), ltt.summation(A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
...
def test_summation_backward(shape, axes, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    backward_check(ltt.summation, A, axes=axes)
...
def test_broadcast_to(shape, shape_to, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.broadcast_to(_A, shape_to), ltt.broadcast_to(A, shape_to).numpy(), atol=1e-5, rtol=1e-5)
...
def test_reshape(shape, shape_to, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ltt.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.reshape(_A, shape_to), ltt.reshape(A, shape_to).numpy(), atol=1e-5, rtol=1e-5)
...
def test_transpose(shape, axes, device):
    ...
    if axes is None:
        ...
        np_axes = axes
    np.testing.assert_allclose(np.swapaxes(_A, np_axes[0], np_axes[1]), ltt.transpose(A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
...
def test_logsumexp(shape, axes, device):
    ...
    if axes is None:
        ...
        t_axes = axes
    np.testing.assert_allclose(torch.logsumexp(A_t, dim=t_axes).numpy(), ltt.logsumexp(A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
