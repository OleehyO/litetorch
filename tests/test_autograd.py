...
def test_power_scalar_forward():
    np.testing.assert_allclose(
        ltt.power_scalar(ltt.Tensor([[0.5, 2.0, 3.0]]), scalar=2).numpy(),
...
def test_divide_forward():
    np.testing.assert_allclose(
        ltt.divide(
            ltt.Tensor([[3.3, 4.35, 1.2], [2.45, 0.95, 2.55]]),
            ltt.Tensor([[4.6, 4.35, 4.8], [0.65, 0.7, 4.4]]),
        ).numpy(),
        np.array(
            [
...
def test_divide_scalar_forward():
    np.testing.assert_allclose(
        ltt.divide_scalar(ltt.Tensor([[1.7, 1.45]]), scalar=12).numpy(),
...
def test_matmul_forward():
    np.testing.assert_allclose(
        ltt.matmul(
            ltt.Tensor([[4.95, 1.75, 0.25], [4.15, 4.25, 0.3], [0.3, 0.4, 2.1]]),
            ltt.Tensor([[1.35, 2.2, 1.55], [3.85, 4.8, 2.6], [1.15, 0.85, 4.15]]),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ltt.matmul(
            ltt.Tensor([[3.8, 0.05], [2.3, 3.35], [1.6, 2.6]]),
            ltt.Tensor([[1.1, 3.5, 3.7], [0.05, 1.25, 1.0]]),
        ).numpy(),
        np.array(
    ...
    np.testing.assert_allclose(
        ltt.matmul(
            ltt.Tensor([[1.9, 1.9], [4.8, 4.9], [3.25, 3.75]]),
            ltt.Tensor(
                [
    ...
    np.testing.assert_allclose(
        ltt.matmul(
            ...
            ltt.Tensor([[2.2, 0.65, 2.5], [2.5, 1.3, 0.15]]),
        ).numpy(),
        np.array(
            [
                [
...
def test_summation_forward():
    ...
    np.testing.assert_allclose(
        ltt.summation(
            ltt.Tensor([[1.5, 3.85, 3.45], [1.35, 1.3, 0.65], [2.6, 4.55, 0.25]]),
            axes=0,
        ).numpy(),
...
def test_broadcast_to_forward():
    np.testing.assert_allclose(
        ltt.broadcast_to(ltt.Tensor([[1.85, 0.85, 0.6]]), shape=(3, 3, 3)).numpy(),
        np.array(
            [
...
def test_negate_forward():
    np.testing.assert_allclose(
        ltt.negate(ltt.Tensor([[1.45, 0.55]])).numpy(), np.array([[-1.45, -0.55]])
    )


def test_transpose_forward():
    np.testing.assert_allclose(
        ltt.transpose(ltt.Tensor([[[1.95]], [[2.7]], [[3.75]]]), axes=(1, 2)).numpy(),
        np.array([[[1.95]], [[2.7]], [[3.75]]]),
    )
    np.testing.assert_allclose(
        ltt.transpose(
            ltt.Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]]), axes=(2, 3)
        ).numpy(),
    ...
    np.testing.assert_allclose(
        ltt.transpose(ltt.Tensor([[[2.45]], [[3.5]], [[0.9]]]), axes=(0, 1)).numpy(),
        np.array([[[2.45], [3.5], [0.9]]]),
    )
    np.testing.assert_allclose(
        ltt.transpose(ltt.Tensor([[4.4, 2.05], [1.85, 2.25], [0.15, 1.4]])).numpy(),
        np.array([[4.4, 1.85, 0.15], [2.05, 2.25, 1.4]]),
    )
    np.testing.assert_allclose(
        ltt.transpose(
            ltt.Tensor([[0.05, 3.7, 1.35], [4.45, 3.25, 1.95], [2.45, 4.4, 4.5]])
        ).numpy(),
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    if not backward:
        ...
        computed_grads = [
            x.numpy()
            for x in out.op.gradient_as_tuple(ltt.Tensor(np.ones(out.shape)), out)
        ]
    else:
...
def test_power_scalar_backward():
    gradient_check(
        ltt.power_scalar, ltt.Tensor(np.random.randn(5, 4)), scalar=np.random.randint(1)
    )

def test_divide_backward():
    gradient_check(
        ltt.divide,
        ltt.Tensor(np.random.randn(5, 4)),
        ltt.Tensor(5 + np.random.randn(5, 4)),
    )


def test_divide_scalar_backward():
    gradient_check(
        ltt.divide_scalar, ltt.Tensor(np.random.randn(5, 4)), scalar=np.random.randn(1)
    )


def test_matmul_simple_backward():
    gradient_check(
        ltt.matmul, ltt.Tensor(np.random.randn(5, 4)), ltt.Tensor(np.random.randn(4, 5))
    )
...
def test_matmul_batched_backward():
    gradient_check(
        ltt.matmul,
        ltt.Tensor(np.random.randn(6, 6, 5, 4)),
        ltt.Tensor(np.random.randn(6, 6, 4, 3)),
    )
    gradient_check(
        ltt.matmul,
        ltt.Tensor(np.random.randn(6, 6, 5, 4)),
        ltt.Tensor(np.random.randn(4, 3)),
    )
    gradient_check(
        ltt.matmul,
        ltt.Tensor(np.random.randn(5, 4)),
...
def test_reshape_backward():
    gradient_check(ltt.reshape, ltt.Tensor(np.random.randn(5, 4)), shape=(4, 5))


def test_negate_backward():
    gradient_check(ltt.negate, ltt.Tensor(np.random.randn(5, 4)))


def test_transpose_backward():
    gradient_check(ltt.transpose, ltt.Tensor(np.random.randn(3, 5, 4)), axes=(1, 2))
    gradient_check(ltt.transpose, ltt.Tensor(np.random.randn(3, 5, 4)), axes=(0, 1))


def test_broadcast_to_backward():
    gradient_check(ltt.broadcast_to, ltt.Tensor(np.random.randn(3, 1)), shape=(3, 3))
    gradient_check(ltt.broadcast_to, ltt.Tensor(np.random.randn(1, 3)), shape=(3, 3))
    gradient_check(
        ltt.broadcast_to,
        ltt.Tensor(
            np.random.randn(
    ...
    gradient_check(ltt.broadcast_to, ltt.Tensor(np.random.randn()), shape=(3, 3, 3))
    gradient_check(
        ltt.broadcast_to, ltt.Tensor(np.random.randn(5, 4, 1)), shape=(5, 4, 3)
    )


def test_summation_backward():
    gradient_check(ltt.summation, ltt.Tensor(np.random.randn(5, 4)), axes=(1,))
    gradient_check(ltt.summation, ltt.Tensor(np.random.randn(5, 4)), axes=(0,))
    gradient_check(ltt.summation, ltt.Tensor(np.random.randn(5, 4)), axes=(0, 1))
    gradient_check(ltt.summation, ltt.Tensor(np.random.randn(5, 4, 1)), axes=(0, 1))
...
def test_topo_sort():
    # Test case 1
    a1, b1 = ltt.Tensor(np.asarray([[0.88282157]])), ltt.Tensor(
    ...
    topo_order = np.array([x.numpy() for x in ltt.autograd.find_topo_sort([c1])])

    assert len(soln) == len(topo_order)
    np.testing.assert_allclose(topo_order, soln, rtol=1e-06, atol=1e-06)

    # Test case 2
    a1, b1 = ltt.Tensor(np.asarray([[0.20914675], [0.65264178]])), ltt.Tensor(
    ...
    topo_order = [x.numpy() for x in ltt.autograd.find_topo_sort([c1])]
    ...
    # Test case 3
    a = ltt.Tensor(np.asarray([[1.4335016, 0.30559972], [0.08130171, -1.15072371]]))
    b = ltt.Tensor(np.asarray([[1.34571691, -0.95584433], [-0.99428573, -0.04017499]]))
    e = (a @ b + b - a) @ a

    topo_order = np.array([x.numpy() for x in ltt.autograd.find_topo_sort([e])])
...
def test_compute_gradient():
    gradient_check(
        lambda A, B, C: ltt.summation((A @ B + C) * (A @ B), axes=None),
        ltt.Tensor(np.random.randn(10, 9)),
        ltt.Tensor(np.random.randn(9, 8)),
        ltt.Tensor(np.random.randn(10, 8)),
        backward=True,
    )
    gradient_check(
        lambda A, B: ltt.summation(ltt.broadcast_to(A, shape=(10, 9)) * B, axes=None),
        ltt.Tensor(np.random.randn(10, 1)),
        ltt.Tensor(np.random.randn(10, 9)),
        backward=True,
    )
    gradient_check(
        lambda A, B, C: ltt.summation(
            ltt.reshape(A, shape=(10, 10)) @ B / 5 + C, axes=None
        ),
        ltt.Tensor(np.random.randn(100)),
        ltt.Tensor(np.random.randn(10, 5)),
        ltt.Tensor(np.random.randn(10, 5)),
        backward=True,
    )
...
def test_softmax_loss_ltt():
    # test forward pass for log
    np.testing.assert_allclose(
        ltt.log(ltt.Tensor([[4.0], [4.55]])).numpy(),
        np.array([[1.38629436112], [1.515127232963]]),
    )

    # test backward pass for log
    gradient_check(ltt.log, ltt.Tensor(1 + np.random.rand(5, 4)))

    X, y = parse_mnist(
        "data/train-images-idx3-ubyte.gz", "data/train-labels-idx1-ubyte.gz"
    )
    np.random.seed(0)
    Z = ltt.Tensor(np.zeros((y.shape[0], 10)).astype(np.float32))
    y_one_hot = np.zeros((y.shape[0], 10))
    y_one_hot[np.arange(y.size), y] = 1
    y = ltt.Tensor(y_one_hot)
    np.testing.assert_allclose(
        softmax_loss(Z, y).numpy(), 2.3025850, rtol=1e-6, atol=1e-6
    )
    Z = ltt.Tensor(np.random.randn(y.shape[0], 10).astype(np.float32))
    np.testing.assert_allclose(
        softmax_loss(Z, y).numpy(), 2.7291998, rtol=1e-6, atol=1e-6
    )

    # test softmax loss backward
    Zsmall = ltt.Tensor(np.random.randn(16, 10).astype(np.float32))
    ysmall = ltt.Tensor(y_one_hot[:16])
...
def test_nn_epoch_ltt():
    ...
    gradient_check(ltt.relu, ltt.Tensor(np.random.randn(5, 4)))
    ...
    dW1 = nd.Gradient(
        lambda W1_: softmax_loss(
            ltt.relu(X_ @ ltt.Tensor(W1_).reshape((5, 10))) @ W2, y_
        ).numpy()
    )(W1.numpy())
    dW2 = nd.Gradient(
        lambda W2_: softmax_loss(
            ltt.relu(X_ @ W1) @ ltt.Tensor(W2_).reshape((10, 3)), y_
        ).numpy()
    )(W2.numpy())
    ...
    np.random.seed(0)
    W1 = ltt.Tensor(np.random.randn(X.shape[1], 100).astype(np.float32) / np.sqrt(100))
    W2 = ltt.Tensor(np.random.randn(100, 10).astype(np.float32) / np.sqrt(10))
    ...
    np.testing.assert_allclose(
        loss_err(ltt.relu(ltt.Tensor(X) @ W1) @ W2, y),
